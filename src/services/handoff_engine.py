...
class HandoffExecutor:
    ...
    async def _create_ticket(self, context: HandoffContext) -> uuid.UUID:
        ...
        # TODO(MAX): persist this payload through the support DB repository.
        # The production database credentials/schema are not available yet.
        ticket_data = {
            "id": str(ticket_id),
    ...
    async def _notify_admins(self, summary: str, ticket_id: uuid.UUID) -> bool:
        ...
        """
        try:
            # TODO(MAX): send through an injected Telegram adapter after the
            # approval-chat credentials and allowlist are configured.
            if self.admin_chat_id is None:
                logger.warning(
                    "Handoff notification skipped: approval chat is not configured"
                )
                return False
            logger.info(
            ...
            logger.warning("Handoff ticket was created but not delivered to Telegram")
            return False
